...
**wingui.dll** — это супер-легковесная графическая библиотека, написанная на чистом **Win32 API (C)**. Она создана специально для того, чтобы стать графическим движком для кастомных интерпретаторов (например, твоего языка программирования) или для использования в минималистичных C/C++ приложениях без тяжелых фреймворков.
